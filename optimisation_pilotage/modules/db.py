...
def list_meetings(theme_id=None, project_id=None, search=None):
    conn = get_conn()
    q = "SELECT id, theme_id, project_id, title, date, participants, source, transcript_path, summary, created_at FROM meetings WHERE 1=1"
    params = []
    if theme_id:
        q += " AND theme_id=?"; params.append(theme_id)
    if project_id:
        q += " AND project_id=?"; params.append(project_id)
    if search:
        q += " AND (title LIKE ? OR summary LIKE ?)"; params.extend([f"%{search}%", f"%{search}%"])
    q += " ORDER BY date DESC, id DESC"
    return get_conn().execute(q, tuple(params)).fetchall()
...
def add_note(meeting_id, content):
    conn = get_conn()
    with conn:
        conn.execute("INSERT INTO notes(meeting_id, content, created_at) VALUES (?,?,?)", (meeting_id, content, now_ts()))

def get_notes(meeting_id):
    conn = get_conn()
    return conn.execute("SELECT id, content, created_at FROM notes WHERE meeting_id=? ORDER BY id DESC", (meeting_id,)).fetchall()
...
def create_todo(theme_id, project_id, meeting_id, action, actor, due_date, status="Ouvert"):
    ...
    with conn:
        cur = conn.execute("""
            INSERT INTO todos(theme_id, project_id, meeting_id, action, actor, due_date, status, created_at)
            VALUES (?,?,?,?,?,?,?,?)
        """,(theme_id, project_id, meeting_id, action, actor, due_date, status, now_ts()))
        return cur.lastrowid

def list_todos(theme_id=None, project_id=None, status=None, search=None):
    conn = get_conn()
    q = "SELECT id, theme_id, project_id, meeting_id, action, actor, due_date, status, created_at FROM todos WHERE 1=1"
    params=[]
    if theme_id: q+=" AND theme_id=?"; params.append(theme_id)
    if project_id: q+=" AND project_id=?"; params.append(project_id)
    if status and status!="Tous": q+=" AND status=?"; params.append(status)
    if search:
        q+=" AND (action LIKE ? OR actor LIKE ?)"; params.extend([f"%{search}%", f"%{search}%"])
