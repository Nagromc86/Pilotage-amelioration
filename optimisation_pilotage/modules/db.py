...
def list_meetings(theme_id=None, project_id=None, search=None):
    conn = get_conn()
    q = "SELECT id, theme_id, project_id, title, date, participants, source, created_at, transcript_path, summary FROM meetings WHERE 1=1"
    args = []
    if theme_id: q += " AND theme_id=?"; args.append(theme_id)
    if project_id: q += " AND project_id=?"; args.append(project_id)
    if search: q += " AND (title LIKE ? OR summary LIKE ?)"; args += [f"%{search}%", f"%{search}%"]
...
def list_todos(theme_id=None, project_id=None, status=None, search=None):
    conn = get_conn()
    q = "SELECT id, theme_id, project_id, meeting_id, action, actor, COALESCE(due_date,''), status, created_at FROM todos WHERE 1=1"
    args = []
    if theme_id: q += " AND theme_id=?"; args.append(theme_id)
    if project_id: q += " AND project_id=?"; args.append(project_id)
    if status: q += " AND status=?"; args.append(status)
    if search: q += " AND (action LIKE ? OR actor LIKE ?)"; args += [f"%{search}%", f"%{search}%"]
