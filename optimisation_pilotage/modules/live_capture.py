...
class LiveTranscriber:
    ...
    def _open_model(self):
        # priorité au modèle local
        local_small = MODELS_DIR / "faster-whisper-small"
        local_medium = MODELS_DIR / "faster-whisper-medium"
        model_path = None
        if self.cfg.model_size == "small" and local_small.exists():
            model_path = str(local_small)
        elif self.cfg.model_size == "medium" and local_medium.exists():
            model_path = str(local_medium)
        else:
            model_path = self.cfg.model_size  # téléchargement si nécessaire
    ...
    def _run(self):
        ...
        try:
            ...
            mic = self._pick_mic()
            spk = self._pick_speaker()
            if mic is None or spk is None:
                raise RuntimeError("Aucun micro ou haut-parleur détecté (vérifiez les périphériques audio Windows).")
            ...
            spk_rec.__exit__(None, None, None)
