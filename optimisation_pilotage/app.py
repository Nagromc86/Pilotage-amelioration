...
class MainWindow:
    def __init__(self, root):
        ...
        root.title('CHAP1 – Compte-rendus Harmonises et Assistance au Pilotage 1 (v2.4.5)'); root.geometry('1150x760')
        self._build_ui();
    ...
    def _start_live(self):
        try:
            ...
            mic=parse_idx(self.cbo_mic.get()); sysd=parse_idx(self.cbo_sys.get())
            if mic is None and sysd is None:
                messagebox.showinfo('Périphériques requis', 'Sélectionnez au moins un périphérique (micro ou système).'); return

            self.state=AppState(); self.txt.delete('1.0','end'); self.var_status.set(f'Enregistrement… (pas {chunk}s)'); self.btn_toggle.configure(text='Arrêter'); self._live_on=True
