...
class AppState:
    def __init__(self):
        self.transcript=''; self.current_meeting_id=None; self.word_count=0
        self.autosave_path = utils.AUTOSAVE_DIR / 'live_autosave.txt'
class MainWindow:
    def __init__(self, root):
        self.root=root; db.init_db()
        self.model_mgr = models_manager.ModelsManager(on_change=self._on_model_change)
        root.title('CHAP1 – Compte-rendus Harmonises et Assistance au Pilotage 1 (v2.4.1)'); root.geometry('1150x720')
        self._build_ui(); self._start_autosave()
    def _build_ui(self):
    ...
    def _build_tab_live(self):
        ...
        ttk.Label(mid, text='Micro:').pack(side='left', padx=(16,0)); self.cbo_mic=ttk.Combobox(mid, width=32); self.cbo_mic.pack(side='left', padx=6)
        ttk.Label(mid, text='Système:').pack(side='left', padx=(16,0)); self.cbo_sys=ttk.Combobox(mid, width=32); self.cbo_sys.pack(side='left', padx=6)
    ...
    def _build_tab_cr(self):
        ...
        for c in cols_cr: self.tree_cr.heading(c, text=c.capitalize()); self.tree_cr.column(c, width=140 if c!='title' else 240)
        ...
        for c in cols_td: self.tree_td.heading(c, text=c.capitalize()); self.tree_td.column(c, width=120 if c not in ('action',) else 320)
        self.tree_td.pack(fill='both', expand=True); self._refresh_tables()
    ...
    def _build_tab_settings(self):
        ...
        ttk.Label(fold, text=f'CR (DATA): {utils.DATA_DIR}').pack(anchor='w', padx=8, pady=2)
        ...
        ttk.Label(fold, text=f'Autosave: {utils.AUTOSAVE_DIR}').pack(anchor='w', padx=8, pady=2)
        ttk.Button(frm, text='Ouvrir logs', command=lambda: self._open_folder(utils.LOGS_DIR)).pack(anchor='w', pady=8, padx=4)
        self._on_model_change()
    def _fill_devices(self):
        try:
            devs=audio_mix.LiveMixer.list_devices()
            names=[f"{i}: {d['name']}" for i,d in enumerate(devs)]
            loopbacks=[f"{i}: {d['name']}" for i,d in enumerate(devs) if 'loopback' in d['name'].lower() or 'wasapi loopback' in d['name'].lower()]
            self.cbo_mic['values']=names; self.cbo_sys['values']=loopbacks or names
        except Exception:
    ...
    def _start_live(self):
        try:
            ...
            fname=f"{d}_{utils.safe_filename(thematique)}_{utils.safe_filename(projet)}_{utils.safe_filename(title)}.txt"
            self.current_cr_path = utils.DATA_DIR / fname
            ms=self.model_mgr.medium if size=='medium' else self.model_mgr.small
            if not ms.present: messagebox.showwarning('Modèle manquant', f'Modèle {size} non installé. Téléchargez-le dans Paramètres.'); return
            self.transcriber=wt.Transcriber(utils.MODELS_DIR, size=size)
            mic=self.cbo_mic.get().split(':')[0]; mic=int(mic) if mic.isdigit() else None
            sysd=self.cbo_sys.get().split(':')[0]; sysd=int(sysd) if sysd.isdigit() else None
            self.state=AppState(); self.txt.delete('1.0','end'); self.var_status.set('Enregistrement en cours...'); self.btn_toggle.configure(text='Arrêter'); self._live_on=True
            def on_chunk(path: Path):
                try:
                    ...
                    if text:
                        ...
                        self.var_wc.set(f"Mots: {len(self.state.transcript.split())}")
                finally:
                    try: path.unlink(missing_ok=True)
                    except Exception: pass
            self.mixer=audio_mix.LiveMixer(chunk_seconds=chunk, mic_device=mic, sys_device=sysd, on_chunk=on_chunk, out_dir=utils.AUTOSAVE_DIR); self.mixer.start()
    ...
    def _stop_live(self):
        ...
        thematique=self.ent_thematique.get().strip(); projet=self.ent_projet.get().strip()
        title=self.ent_title.get().strip() or 'SansTitre'; d=self.ent_date.get().strip() or utils.today_str()
        mid=db.add_meeting(d, thematique, projet, title, '', getattr(self.state,'transcript',''), ''); self._refresh_tables()
    def _save_cr(self):
        if not hasattr(self,'current_cr_path'): self.current_cr_path = utils.DATA_DIR / f"{utils.today_str()}_CR.txt"
        self.current_cr_path.write_text(self.txt.get('1.0','end').strip(), encoding='utf-8'); messagebox.showinfo('Sauvegarde', f'CR enregistré : {self.current_cr_path}')
    def _status_str(self, ms): return f"{ms.status_text} ({ms.progress}%)"
    ...
    def _refresh_tables(self):
        ...
        for row in meetings: self.tree_cr.insert('', 'end', values=row[:6])
        todos=db.list_todos()
        for row in todos:
            tag = 'warn' if (not row[5]) or (not row[6]) else ''
    ...
    def _edit_participants(self):
        ...
        if not sel: messagebox.showwarning('Sélection','Sélectionnez un CR.'); return
        vals=self.tree_cr.item(sel[0],'values'); mid=int(vals[0])
        row=db.get_meeting(mid); init=row[5] if row and len(row)>=6 else ''
        win=tk.Toplevel(self.root); win.title('Éditer participants'); win.geometry('520x160')
        ttk.Label(win, text='Participants (séparés par ,):').pack(anchor='w', padx=10, pady=6)
        ent=ttk.Entry(win, width=70); ent.pack(fill='x', padx=10); ent.insert(0, init or '')
        def save(): db.update_meeting(mid, participants=ent.get().strip()); self._refresh_tables(); win.destroy()
        ttk.Button(win, text='Enregistrer', command=save).pack(pady=10)
    def _edit_cr(self):
        sel=self.tree_cr.selection()
        if not sel: messagebox.showwarning('Sélection','Sélectionnez un CR.'); return
        vals=self.tree_cr.item(sel[0],'values'); mid=int(vals[0])
        row=db.get_meeting(mid); init=row[6] if row and len(row)>=7 else ''
        win=tk.Toplevel(self.root); win.title('Éditer CR'); win.geometry('800x500')
        txt=tk.Text(win, wrap='word'); txt.pack(fill='both', expand=True); txt.insert('1.0', init or '')
        def save(): db.update_meeting(mid, content=txt.get('1.0','end').strip()); self._refresh_tables(); win.destroy()
        ttk.Button(win, text='Enregistrer', command=save).pack(pady=6)
    def _do_export(self):
        path = filedialog.asksaveasfilename(defaultextension='.xlsx', filetypes=[('Excel','.xlsx')], initialdir=str(utils.EXPORTS_DIR), initialfile='CHAP1_export.xlsx')
        if not path: return
        meetings=db.list_meetings(); todos=db.list_todos()
        export_mod.export_excel(Path(path), meetings, todos); messagebox.showinfo('Export', f'Exporté vers {path}')
            ...
            while True:
                ...
                try:
                    if hasattr(self,'txt'):
                        ...
                        if t: self.state.autosave_path.write_text(t, encoding='utf-8')
                except Exception: pass
